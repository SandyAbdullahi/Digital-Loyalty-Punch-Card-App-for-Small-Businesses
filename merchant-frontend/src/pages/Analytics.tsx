  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="font-heading text-3xl font-semibold text-rudi-maroon">Analytics</h1>
        <p className="text-sm text-rudi-maroon/70">
          Track loyalty momentum and spotlight your top-performing programs.
        </p>
      </div>

      <div className="flex flex-col gap-3 rounded-3xl bg-white/60 p-5 shadow-rudi-card md:flex-row md:items-end md:justify-between">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row">
          <div className="flex-1 space-y-1.5">
            <Label className="text-xs font-semibold uppercase text-rudi-maroon/70">From</Label>
            <Input
              type="date"
          ...
          <div className="flex-1 space-y-1.5">
            <Label className="text-xs font-semibold uppercase text-rudi-maroon/70">To</Label>
            <Input
              type="date"
              value={dateRange.to}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="card-hover rounded-3xl bg-white p-6 shadow-rudi-card animate-slide-up">
          <h2 className="font-heading text-lg text-rudi-maroon">Stamps vs. Redemptions</h2>
          ...
          <div className="mt-6 flex justify-center">
            <svg
              ...
              className="w-full max-w-lg text-rudi-maroon/50"
            >
              <rect
        ...
        <div className="card-hover rounded-3xl bg-white p-6 shadow-rudi-card animate-slide-up" style={{ animationDelay: '0.05s' }}>
          <h2 className="font-heading text-lg text-rudi-maroon">Active customers per day</h2>
          <p className="text-xs text-rudi-maroon/60">
            ...
            {activeCustomersData.map((item) => {
              ...
              return (
                <div key={item.day} className="flex flex-1 flex-col items-center gap-2">
                  <div
                    className="w-full rounded-t-2xl bg-rudi-yellow transition-all duration-200"
                    style={{ height: `${height}%` }}
                  />
                  <span className="text-xs text-rudi-maroon/60">{item.day}</span>
                </div>
              )
            })}
          </div>
        ...
        <div
          className="card-hover rounded-3xl bg-white p-6 shadow-rudi-card animate-slide-up md:col-span-2 lg:col-span-1"
          style={{ animationDelay: '0.1s' }}
        >
          <h2 className="font-heading text-lg text-rudi-maroon">Top performing programs</h2>
          <p className="text-xs text-rudi-maroon/60">
          ...
          <div className="mt-6 flex flex-col items-center gap-4 lg:flex-row">
            ...
            >
              <div className="relative h-full w-full rounded-full bg-white/0">
                <div className="absolute left-1/2 top-1/2 h-28 w-28 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/90 shadow-inner" />
              </div>
            </div>
            <div className="space-y-2 text-sm text-rudi-maroon/70">
              {programPerformance.map((program, index) => (
                <div key={program.name} className="flex items-center gap-2">
                  ...
                  <span className="ml-auto font-semibold text-rudi-maroon">{program.value}%</span>
                </div>
              ))}
            </div>
          </div>
