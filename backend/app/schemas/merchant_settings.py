...
class MerchantSettingsBase(BaseModel):
    avg_spend_per_visit_kes: float = Field(default=0, ge=0, description="Average spend per visit in KES")
    baseline_visits_per_customer_per_period: float = Field(default=0, ge=0, description="Baseline visits per customer per period")
    avg_reward_cost_kes: float = Field(default=0, ge=0, description="Average reward cost in KES")
